...
#################################################################################################################
packages_required <- c("tidyverse","lubridate","DSSAT","furrr","future","future.apply","stringr","geodata")
...
# Function to add fertilizer levels and treatments to DSSAT experimental file
populate_fert_n_trt_factorial <- function(file_x, fert_df, plant_dates, 
                                          file_x_original) {
  ...
  for (i in seq_along(plant_dates)){
    for (j in 1:max(fert_df$F)){  # Loop through all fertilizer levels
      f_ij <- f_ij + 1
...
create_filex <-function(i, path.to.temdata, filex_temp, path.to.extdata, coords,
                        ...
                        fert_factorial = FALSE, geneticfiles, index_soilwat = 1,
                        wsta_prefix = "WHTE", fert_file = "fert_rec.csv", 
                        plant_dates = NULL){
  ...
  # Read fertilizer experiments from Landing
  if (fert_factorial){
    file_x_original <- DSSAT::read_filex(filex_temp)
    fert_df <- read_csv(fert_file, show_col_types = FALSE, progress = FALSE)
  }
  ...
  if (!is.null(plant_dates) && length(plant_dates) >= 1 && !all(is.na(plant_dates))) {
    ...
    if (fertilizer & !fert_factorial) {
      ...
      file_x <- populate_fert_n_trt_factorial(
        file_x = file_x, fert_df = fert_df, plant_dates = plant_dates, 
...
dssat.expfile <- function(country, useCaseName, Crop, AOI = TRUE, filex_temp,
                          ...
                          zone, level2 = NA, fertilizer = FALSE, 
                          fert_factorial = FALSE, fert_file = "fert_rec.csv",
  ...
  if(AOI == TRUE){
    path.to.extdata <- paste("/home/jovyan/agwise-planting-date-and-cultivar/Data/useCase_", country, "_",useCaseName, "/", Crop, "/transform/DSSAT/AOI/",varietyid, sep="")
  }else{
    path.to.extdata <- paste("/home/jovyan/agwise-planting-date-and-cultivar/Data/useCase_", country, "_",useCaseName, "/", Crop, "/transform/DSSAT/fieldData/",varietyid, sep="")
  }
  if (!dir.exists(path.to.extdata)) dir.create(path.to.extdata, recursive = TRUE)

  path.to.temdata <- paste("/home/jovyan/agwise-planting-date-and-cultivar/Data/useCase_", country, "_",useCaseName, "/", Crop, "/Landing/DSSAT", sep="")
